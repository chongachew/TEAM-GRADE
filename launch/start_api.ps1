# TEAM-GRADE API Launcher
# Activates venv and runs the API server
...
Write-Host "TEAM-GRADE API Server Startup" -ForegroundColor Cyan
...
# Run API
try {
    Write-Host "$WORK Launching API server module..." -ForegroundColor Yellow
    python api\server.py
} catch {
    Write-Host "$FAIL Failed to start API: $_" -ForegroundColor Red
    exit 1
}

# Keep window open if script exits
Write-Host "`n$SEPARATOR" -ForegroundColor Yellow
Write-Host "[INFO] API server has stopped. This window will remain open." -ForegroundColor Yellow
