"""
Whistle detection module.

Ported from a standalone `preprocessor/` proof-of-concept PR that was never
wired into the real pipeline - ported as-is (algorithm unchanged) into this
package's convention (standalone processing modules, importable independently
of the Firestore-backed pipeline - see rep_extraction.py, torso_cropper.py),
with settings now sourced from config.settings instead of a separate
preprocessor/config.py, matching how every other stage in this codebase
